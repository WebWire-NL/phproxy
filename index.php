...
/*
   ...
   |  Script         | PHProxy                                                       |
   |  Author         | WebWire                                                       |
   |  Last Modified  | Thu Nov 25 05:27:58 AM UTC 2021                               |
   |  Credits	     | Original script writer unknown (http://www.whitefyre.com/poxy)|
   +-----------------+---------------------------------------------------------------+
...
$_version           = '0.5.1';
$_http_host         = isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : (isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : 'localhost');
...
function url_parse($url, & $container)
{
    ...
    if (!empty($temp))
    {
        ...
        {
            $temp['port'] = $temp['scheme'] === 'https' ? 443 : $_SERVER["SERVER_PORT"];
        }
...
function complete_url($url, $proxify = true)
{
    ...
    $ret=$proxify ? "{$GLOBALS['_script_url']}?{$GLOBALS['_config']['url_var_name']}=" . encode_url($url) . $fragment : $url;
    return $ret;
}
...
unset($_response_headers['content-security-policy']);
foreach ($_response_headers as $h) {
}
$_response_headers   = array_filter($_response_headers);
$_response_keys      = array_filter($_response_keys);
